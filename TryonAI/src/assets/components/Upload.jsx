...
const Upload = ({ selectedCloth, scrollToClothesSelector, setSelectedImage }) => {
  ...
  useEffect(() => {
    if (selectedCloth && uploadedImage) {
    ...
    <div className={`container mx-auto text-center p-6 relative ${isLoading && 'opacity-50'}`}>
      ...
      <div className="flex justify-center items-center mt-10 space-x-20">
        ...
        <div className="border border-dashed border-black p-6 w-[28rem] h-[25rem] flex flex-col justify-center items-center">
          ...
          <img src={uploadedImage ? Added : Add} alt="Logo" className={`w-[6rem] h-[6rem] transition-opacity duration-300 ${uploadedImage ? 'opacity-100' : 'opacity-50'}`} />
          <input
            type="file"
            id="upload"
          ...
          {uploadedImageName && (
            <p className="text-gray-500 mt-4">{uploadedImageName}</p>
          )}
